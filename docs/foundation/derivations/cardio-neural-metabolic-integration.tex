...
PPG dataset (Oura Ring), (2) the MIT-BIH Arrhythmia Database (PhysioNet,
48 records, 360\,Hz ECG), and (3) the PhysioNet Congestive Heart Failure
RR Interval Database (15 records). All results are archived in JSON and CSV
...
circular dispersion formula for Kuramoto oscillators
...
This is the well-known \emph{loss of complexity} phenomenon in cardiology
(Goldberger et al., 2002), here derived from first principles. The
...
from normative EEG spectral power (Rechtschaffen \& Kales, 1968;
AASM Manual, 2007):
...
\item \textbf{AFIB as turbulent regime}: $R_c(\text{AFIB}) < 0.30$.
...
\item \textbf{Meditation effect}: 30\,min meditation shifts $R_n$ without $R_c$ change.
...
\item \textbf{General anesthesia as forced phase-locking}: Propofol drives
  $R_n \to 0.95$. \textbf{[PENDING]} Requires intraoperative EEG data.
