...
camera. Layer~0 is the air--skin interface;
...
% Note: the layered-model schematic is described inline in the text;
% all figures in this paper are data-driven 4-up panels (see captions
% file figures/optical-captions.tex).
...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{figures/panel1_forward_sweeps.png}
\caption{Forward-model RGB reflectance as a function of each state
parameter, holding others at reference. Blue: melanin sensitivity is
dominant in the B channel, modest in G, weak in R. Red: hemoglobin
sensitivity is strong in G and B, weaker in R. Green: oxygenation
sensitivity acts only in G. Vasodilation: amplifies the dermal
absorption pathway and is therefore visible in all channels but most
strongly in G.}
\label{fig:forward-spectra}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{figures/panel2_self_consistency.png}
\caption{Per-parameter recovery error histograms for $1{,}000$
random physiological states under noise-free conditions. Vasodilation
is exact by construction (supplied as input). Melanin recovers
sharply (median $0.5\%$). Hb recovers to median $10\%$ with a
long upper tail at extreme parameter combinations. Oxygenation
recovery is dominated by the small spectral selectivity of the
green channel between oxy- and deoxyhemoglobin and produces a
broad error distribution.}
...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{figures/panel3_noise_sensitivity.png}
\caption{RMS recovery error per parameter against additive
Gaussian noise SNR. Melanin and Hb are noise-limited and improve
with SNR. Oxygenation is model-limited at $\approx 35\%$ RMS for
all SNR, confirming that its floor is set by the band-averaged
spectral approximation rather than by measurement noise.}
...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{figures/panel4_vaso_temperature.png}
\caption{Vasodilation--temperature linearisation
(\cref{eq:vaso-T}) overlaid with empirical forehead temperature
ranges from cold-pressor (lower band), quiet sitting (centre), mild
exercise (upper-mid), and maximum exercise (top). The mapping
captures the physiologically plausible range and slope.}
\label{fig:vaso-temp}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{figures/panel5_pchr_scenarios.png}
\caption{Stacked-bar PCHR decomposition for the four scenarios.
Rest establishes the baseline; fever activates the metabolic
component; hypoxia activates the hypoxic component; exercise
activates the autonomic component. Each scenario's primary drive is
correctly identified by the dominant non-baseline contribution.}
...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{figures/panel6_conventional_comparison.png}
\caption{Left: HR estimation variance vs. ambient-light drift
amplitude for the green-only baseline (method A) and the
DC-normalised layered method (method B). The two methods perform
comparably: the cardiac-band band-pass rejects the slow drift in
both cases. Right: SpO$_{2}$ recovery error of the layered
method's full state inversion as a function of drift amplitude;
the green-only baseline produces no SpO$_{2}$ estimate. The
layered method's distinct value is the additional state recovery,
not HR variance.}
\label{fig:conventional-comp}
\end{figure}
